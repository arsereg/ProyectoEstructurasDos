...
export class NodoComponent implements OnInit {
  ...
  trackId(index: number, item: INodo): number {
    return item.id!;
  }

  delete(nodo: INodo): void {
    const modalRef = this.modalService.open(NodoDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: INodo[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
